...
const Header = ({ setToggleR, authToken, setToggle, toggle }) => {
  ...
  return (
    <div className="w-full fixed top-0 left-0 flex items-center justify-between shadow sm:px-8 px-2 py-3 bg-white z-[100]">
      ...
      {user?.email ? (
        <div>
          <button onClick={()=>{logout()}} className="text-white bg-[#1F847F] hover:bg-[#186965] md:text-[16px] text-sm font-semibold sm:px-8 px-5 py-3 rounded transition-all duration-300 text-nowrap">
          Abmelden
          </button>
        </div>
      ) : (
